...
__all__ = ['format_simple_means', 'format_oxford_entry', 'format_oxford_unbox', 'format_collins', 'with_new_line']
...
def format_simple_means(result):
    ...
    for symbol in symbols:
        ...
        for part in parts:
            line = part.get('part', '')  + '; '.join(part.get('means', []))
            text += line + "\n"
            ...
            for datum in box_data:
                if datum.get('tag') == 'title':
                    ...
                    for i, text_datum in enumerate(text_data):
                        ch_text = text_datum.get('chText')
                        en_text = "* " + text_datum.get('enText')
